...
def create_movie(db: Session, db_movie: models.Movie):
    ...
    try:
        ...
        db.rollback()
        movie = get_movie_by_schema(db, schemas.MovieCreate(name=db_movie.name, engine=db_movie.engine)).first()
...
def create_download(db: Session, download: schemas.DownloadCreate):
    ...
    movie = create_movie(db, movie)
    db_download = models.Download(movie_id=movie.id, ip_address=download.ip_address, datetime=datetime.datetime.utcnow())
    db.add(db_download)
    db.commit()
...
def get_highest_downloads(db: Session, filter_: schemas.DownloadFilter):
    ...
    downloads_in_the_last_x_times = db.query(models.Download).filter(models.Download.datetime > x_times_ago).all()
...
def create_referral(db: Session, referral: schemas.ReferralCreate):
    ...
    movie = create_movie(db, movie)
    db_referral = models.Referral(movie_id=movie.id, ip_address=referral.ip_address, datetime=datetime.datetime.utcnow())
    db.add(db_referral)
    db.commit()
